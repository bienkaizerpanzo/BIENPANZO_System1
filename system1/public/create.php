...
<body>
    <div class="form-box">
        ...
        <form method="POST">
            ...
            <input type="text" name="pet_name" value="<?= htmlspecialchars($_POST['pet_name'] ?? '') ?>" required>
            ...
            <select name="pet_type_id" required>
                ...
                <?php foreach ($petTypes as $petType): ?>
                    <?php
                        ...
                        $selected = ((int) ($_POST['pet_type_id'] ?? 0) === $optionValue) ? 'selected' : '';
                    ?>
                    <option value="<?= $optionValue ?>" <?= $selected ?>>
                        <?= htmlspecialchars($petType['name'] ?? '') ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <label>Age</label>
            <input type="number" name="age" min="0" value="<?= htmlspecialchars((string) ($_POST['age'] ?? 0)) ?>">
            ...
            <input type="text" name="contact_number" value="<?= htmlspecialchars($_POST['contact_number'] ?? '') ?>" required>

            <label>Status</label>
            <select name="status">
                <option value="Active" <?= (($_POST['status'] ?? 'Active') === 'Active') ? 'selected' : '' ?>>Active</option>
                <option value="Recovered" <?= (($_POST['status'] ?? 'Active') === 'Recovered') ? 'selected' : '' ?>>Recovered</option>
                <option value="Pending" <?= (($_POST['status'] ?? 'Active') === 'Pending') ? 'selected' : '' ?>>Pending</option>
            </select>
